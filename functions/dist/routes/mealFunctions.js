...
function containsBanned(plan, bannedList) {
    if (!Array.isArray(plan) || bannedList.length === 0)
        return false;
    return plan.some(meal => Array.isArray(meal.ingredients) &&
        meal.ingredients.some(ing => bannedList.some(b => ing.item?.toLowerCase().includes(b.toLowerCase()))));
}
// Single Meal
...
Respond ONLY with valid JSON like this:
{
  ...
  "instructions": [
    ...
    try {
        ...
        if (!parsed?.name)
            throw new Error("Invalid single meal data");
        return parsed;
    }
    catch (error) {
...
Respond ONLY with valid JSON object like:
{
  ...
  "instructions": [
    ...
    try {
        ...
        if (!parsed?.name)
            throw new Error("Invalid requested meal data");
        return parsed;
    }
    catch (error) {
